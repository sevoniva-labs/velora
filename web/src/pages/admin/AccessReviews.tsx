...
export default function AccessReviews() {
  ...
  return <PageContainer title="权限复核"><ProTable<AccessReview> className="velora-admin-primary-table" rowKey="id" columns={columns} {...reviewTable} loading={reviews.isLoading} search={{ labelWidth: 'auto' }} pagination={{ pageSize: 20 }} toolBarRender={canManage ? () => [<Button key="create" type="primary" icon={<PlusOutlined />} onClick={() => setCreateOpen(true)}>发起复核</Button>] : false} />
