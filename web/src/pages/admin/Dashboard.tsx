...
export default function AdminDashboard() {
  ...
  return <PageContainer title="工作台">
    ...
    <Row gutter={[16, 16]} style={{ marginTop: canManagePortal ? 16 : 0 }}>
      ...
      {canReadTemporaryGrants && <Col xs={24} lg={8}><ProCard title="正在使用的临时权限" extra={<Button type="link" onClick={() => navigate('/admin/temporary-grants')}>查看</Button>}><Space align="center"><ClockCircleOutlined style={{ fontSize: 24, color: '#d48806' }} /><Typography.Title level={3} style={{ margin: 0 }}>{activeGrants.length}</Typography.Title></Space></ProCard></Col>}
      {canReadReviews && <Col xs={24} lg={8}><ProCard title="待完成的权限检查" extra={<Button type="link" onClick={() => navigate('/admin/access-reviews')}>查看</Button>}><Space align="center"><SafetyCertificateOutlined style={{ fontSize: 24, color: '#389e0d' }} /><Typography.Title level={3} style={{ margin: 0 }}>{openReviews.length}</Typography.Title></Space></ProCard></Col>}
    </Row>
