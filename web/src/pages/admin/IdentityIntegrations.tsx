  ...
  return (
    <div>
      ...
      {selected && onboarding.data ? <Card title={`${selected.name} · 接入向导`}>
        <Alert type={onboarding.data.blockers.length ? 'warning' : onboarding.data.onboardingStatus === 'PUBLISHED' ? 'success' : 'info'} showIcon style={{ marginBottom: 20 }} message={<Space><span>{onboarding.data.onboardingStatus}</span><span>{onboarding.data.nextAction}</span></Space>} description={<Space direction="vertical" size={2}>{onboarding.data.blockers.length ? <span>阻塞项：{onboarding.data.blockers.join('；')}</span> : <span>当前配置没有阻塞项。</span>}{onboarding.data.latestOperation ? <Text type="secondary">最近操作：{onboarding.data.latestOperation.operationType} · {onboarding.data.latestOperation.status} · 尝试 {onboarding.data.latestOperation.attemptCount} 次{onboarding.data.latestOperation.errorCode ? ` · ${onboarding.data.latestOperation.errorCode}` : ''}</Text> : null}</Space>} />
        ...
        {step === 3 ? selected.ssoType === 'URL' ? <Alert type="success" showIcon message="普通链接无需身份配置" description="账号与访问策略保存后可直接进入验证与发布。" /> : !identityTypeSupported ? <Alert type="warning" showIcon message="该接入类型暂未开放" description="当前仅开放 OIDC 与普通链接。" /> : <Form form={form} layout="vertical" disabled={!canManage} onValuesChange={(_, values: IdentityDraft) => { if (!selectedId) return; try { window.localStorage.setItem(draftStorageKey(selectedId), JSON.stringify({ redirectUris: values.redirectUris })) } catch { /* 只保存非敏感回调地址草稿 */ } }} onFinish={(values) => prepareCredentialMutation.mutate(values)}><Descriptions bordered column={1} size="small" style={{ marginBottom: 16 }}><Descriptions.Item label="登录协议">OIDC Authorization Code + PKCE S256</Descriptions.Item><Descriptions.Item label="账号入口">用户始终从 Velora 登录，Casdoor 不直接暴露</Descriptions.Item><Descriptions.Item label="自动编排">Velora 自动生成 Client ID、Issuer 和 Scopes，并在待办中关联审批</Descriptions.Item><Descriptions.Item label="凭据交付">velora-connect 使用 5 分钟单次 Token 领取，浏览器不展示真实 Secret</Descriptions.Item></Descriptions>{!canManage ? <Alert type="info" showIcon style={{ marginBottom: 16 }} message="当前账号仅具备查看权限" /> : null}<Form.Item label="回调地址（每行一个）" name="redirectUris" rules={[{ required: true, message: '请输入回调地址' }]}><Input.TextArea rows={3} placeholder="https://app.example.com/api/v1/auth/oidc/callback" /></Form.Item><Form.Item name="providerApplicationRef" hidden><Input /></Form.Item><Form.Item name="publicClientId" hidden><Input /></Form.Item><Form.Item name="issuer" hidden><Input /></Form.Item><Form.Item name="scopes" hidden><Input /></Form.Item><Button type="primary" htmlType="submit" loading={prepareCredentialMutation.isPending || bindingMutation.isPending}>申请审批并生成接入配置</Button><Paragraph type="secondary" style={{ marginTop: 12 }}>首次点击会自动创建审批待办；审批通过后再次点击即可签发，不需要复制审批编号，也不需要打开 Casdoor。</Paragraph></Form> : null}
        {step === 4 ? <Space direction="vertical" style={{ width: '100%' }} size="middle"><List size="small" header="当前配置版本检查证据" dataSource={onboarding.data.onboardingChecks} locale={{ emptyText: '尚未运行全链路检查' }} renderItem={(item) => <List.Item><Space wrap><Tag color={item.result === 'PASSED' ? 'success' : 'error'}>{item.result === 'PASSED' ? '通过' : '失败'}</Tag><Text>{item.checkType}</Text><Text type="secondary">{item.errorCode || '检查通过'}</Text>{item.requestId ? <Text type="secondary">Request ID: {item.requestId}</Text> : null}</Space></List.Item>} /><Space wrap>{identityTypeSupported ? <Button icon={<SafetyCertificateOutlined />} loading={checksMutation.isPending} onClick={() => checksMutation.mutate()} disabled={!canVerify}>运行全部自动检查</Button> : <Button loading={verifyMutation.isPending} onClick={() => verifyMutation.mutate()} disabled={!canVerify}>验证配置</Button>}<Button onClick={() => submitMutation.mutate()} loading={submitMutation.isPending} disabled={!canPublish || !onboarding.data.canPublish}>提交发布</Button><Button type="primary" onClick={() => publishMutation.mutate()} loading={publishMutation.isPending} disabled={!canPublish || !onboarding.data.canPublish}>发布应用</Button></Space><Paragraph type="secondary">发布门禁要求当前配置版本的访问范围、OIDC Discovery、账号同步签名、重复事件和乱序事件检查全部通过。人工登录与退出验收需在生产证据中确认。</Paragraph></Space> : null}
