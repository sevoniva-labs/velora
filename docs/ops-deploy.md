...
- 证书轮换由 `velora-cert-renew.timer` 每日两次调用 Certbot；HTTP-01 webroot 为 `/opt/velora/prod/runtime/acme`。每个域名单独维护 lineage，deploy hook 将证书原子替换到 `runtime/certs/<domain>`，先执行 Edge `nginx -t`，失败则恢复 `.bak`，成功后仅热加载 Edge。禁止同时启用系统默认 `certbot.timer`，避免并发续期。

验收命令：

```bash
systemctl is-enabled velora-cert-renew.timer
certbot renew --dry-run --no-random-sleep-on-renew
docker exec velora-prod-edge nginx -t
```
