...
export async function getApplicationOnboarding(id: string | number): Promise<ApplicationOnboarding> { const data = record(await apiFetch<unknown>(`/admin/portal/applications/${encodeURIComponent(String(id))}/onboarding`)); return { application: mapApplication(record(data.application)), binding: mapIdentityBinding(data.binding), verifications: listFrom(data, 'verifications').map(mapVerification), canPublish: Boolean(data.canPublish), onboardingStatus: String(data.onboardingStatus ?? 'DRAFT'), nextAction: String(data.nextAction ?? ''), blockers: Array.isArray(data.blockers) ? data.blockers.map(String) : [], roles: listFrom(data, 'roles').map(mapApplicationRole), provisioningTarget: data.provisioningTarget ? mapApplicationProvisioningTarget(data.provisioningTarget) : undefined, onboardingChecks: listFrom(data, 'onboardingChecks').map(mapOnboardingCheck) } }
