...
var basePermissions = []struct{ Key, Name string }{
	...
	{"system.access_review.read", "查看权限复核"}, {"system.access_review.manage", "管理权限复核"},
	{"system.session.read", "查看在线会话"}, {"system.session.revoke", "强制下线会话"},
	{"system.api_token.manage", "管理集成令牌"},
...
func (s *Service) Bootstrap(ctx context.Context, orgKey, orgName, admin, passwordRaw string) error {
	...
	for _, k := range []string{"system.audit.read", "audit.read", "system.audit.export", "system.audit.verify", "system.temporary_grant.read", "system.api_token.manage"} {
...
func builtinMenus(orgID string) []domain.Menu {
	...
	return []domain.Menu{
		...
		{OrganizationID: orgID, Key: "governance.access-reviews", ParentKey: "governance", Name: "权限复核", Route: "/governance/access-reviews", Icon: "SafetyCertificateOutlined", PermissionKey: "system.access_review.read", SortOrder: 43, Status: "ACTIVE", CreatedAt: now, UpdatedAt: now},
		...
		{OrganizationID: orgID, Key: "governance.config-changes", ParentKey: "governance", Name: "配置发布", Route: "/admin/config-changes", Icon: "SafetyCertificateOutlined", PermissionKey: "system.config.read", SortOrder: 45, Status: "ACTIVE", CreatedAt: now, UpdatedAt: now},
