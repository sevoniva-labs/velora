...
func TestHandoffAllowsSSOOnlyBundle(t *testing.T) {
	c, err := cache.New(config.Cache{Provider: "memory", Prefix: "test-sso:"})
	if err != nil {
		t.Fatal(err)
	}
	provider, err := appcrypto.New("standard", "handoff-test-key-material-32-bytes")
	...
	bundle := Bundle{ApplicationID: "app-1", ApplicationCode: "order-center", ClientSecret: strings.Repeat("c", 32)}
	token, _, err := store.Issue(context.Background(), bundle)
	if err != nil {
		t.Fatal(err)
	}
	got, err := store.Consume(context.Background(), token)
	if err != nil || got.ClientSecret != bundle.ClientSecret || got.ProvisioningSecret != "" || got.DirectoryToken != "" {
		t.Fatalf("consume = %#v, %v", got, err)
	}
}

func TestHandoffRejectsPartialSyncBundle(t *testing.T) {
	c, err := cache.New(config.Cache{Provider: "memory", Prefix: "test-partial:"})
	if err != nil {
		t.Fatal(err)
	}
	provider, err := appcrypto.New("standard", "handoff-test-key-material-32-bytes")
	...
	_, _, err = store.Issue(context.Background(), Bundle{ApplicationID: "app-1", ApplicationCode: "order-center", ClientSecret: strings.Repeat("c", 32), ProvisioningSecret: strings.Repeat("p", 32)})
