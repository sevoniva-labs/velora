...
import { createAccessReview, decideAccessReviewItem, listAccessReviewItems, listAccessReviews } from '../../api/admin-platform'
...
export default function AccessReviews() {
  ...
  return <PageContainer title="访问复核"><ProTable<AccessReview> rowKey="id" columns={columns} dataSource={reviews.data ?? []} loading={reviews.isLoading} search={{ labelWidth: 'auto' }} pagination={{ pageSize: 20 }} toolBarRender={() => [<Button key="create" type="primary" icon={<PlusOutlined />} onClick={() => setCreateOpen(true)}>发起复核</Button>]} />
    ...
    <ModalForm<DecisionForm> key={deciding?.id ?? 'decision'} title="提交复核结果" open={Boolean(deciding)} onOpenChange={(value) => !value && setDeciding(undefined)} initialValues={{ decision: 'APPROVE' }} submitter={{ searchConfig: { submitText: '提交', resetText: '取消' } }} onFinish={async (values) => { await decide.mutateAsync(values); return true }}>
      <ProFormRadio.Group name="decision" label="复核结果" options={[{ label: '保留', value: 'APPROVE' }, { label: '撤销', value: 'REVOKE' }]} rules={[{ required: true }]} />
      <ProFormTextArea name="reason" label="原因" fieldProps={{ maxLength: 500, showCount: true }} />
    </ModalForm>
  </PageContainer>
}
