...
export default function ApplicationManagement() {
  ...
  const tabs = [{ key: 'overview', tab: '概览' }, ...(canManagePortal ? [{ key: 'information', tab: '基本信息' }] : []), ...(canReadIdentity ? [{ key: 'login', tab: '登录设置' }] : []), ...(canManagePortal ? [{ key: 'access', tab: '谁可以使用' }] : []), ...(canReadIdentity ? [{ key: 'provisioning', tab: '账号下发' }] : []), ...(canVerify || canPublish ? [{ key: 'release', tab: '上线检查' }] : []), ...(canReadHistory ? [{ key: 'history', tab: '操作记录' }] : [])]
  return <PageContainer className="velora-application-management" title={application.name} subTitle={application.code} onBack={() => navigate('/admin/applications')} tabList={tabs} tabActiveKey={tab} onTabChange={setTab}>
    {tab === 'overview' && application && <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <div className="velora-application-stat-grid"><StatisticCard className="velora-application-stat-card" statistic={{ title: '配置进度', value: enumLabel(APP_LIFECYCLE_LABEL, application.lifecycleStatus, '草稿') }} /><StatisticCard className="velora-application-stat-card" statistic={{ title: '可使用人数', value: canManagePortal ? effectiveAccess.data?.length ?? 0 : '—' }} /><StatisticCard className="velora-application-stat-card" statistic={{ title: '账号下发', value: canReadIdentity ? provisioning.data?.deliveryStatus === 'HEALTHY' ? '正常' : ['DEGRADED', 'FAILED'].includes(provisioning.data?.deliveryStatus ?? '') ? '异常' : provisioning.data ? '等待下发' : '未配置' : '—' }} /></div>
      <ProDescriptions className="velora-application-overview" dataSource={application} column={{ xs: 1, sm: 1, md: 2 }} columns={[{ title: '应用名称', dataIndex: 'name' }, { title: '应用编码', dataIndex: 'code' }, { title: '配置进度', dataIndex: 'lifecycleStatus', render: (_, row) => <Tag color={APP_LIFECYCLE_COLOR[row.lifecycleStatus ?? '']}>{enumLabel(APP_LIFECYCLE_LABEL, row.lifecycleStatus, '草稿')}</Tag> }, { title: '使用状态', dataIndex: 'status', render: (_, row) => <Tag color={row.status === 'ENABLED' ? 'success' : 'default'}>{row.status === 'ENABLED' ? '可用' : '已停用'}</Tag> }, { title: '应用地址', dataIndex: 'homeUrl', render: (_, row) => row.homeUrl || '—' }, { title: '用途', dataIndex: 'description', render: (_, row) => row.description || '—' }]} />
    </Space>}
