...
function mapApplication(value: unknown): Application {
  ...
  return {
    ...
    ssoType: (launchType || 'URL') as Application['ssoType'], owner: String(item.ownerUserName ?? ''), department: String(item.ownerDepartmentName ?? ''), ownerUserId: String(item.ownerUserId ?? ''), ownerDepartmentId: String(item.ownerDepartmentId ?? ''),
...
function mapOnboardingCheck(value: unknown): OnboardingCheck { const item = record(value); return { id: asId(item.id), applicationId: asId(item.applicationId), configVersion: Number(item.configVersion ?? 0), checkType: String(item.checkType ?? ''), result: String(item.result ?? ''), errorCode: String(item.errorCode ?? ''), evidenceJson: String(item.evidenceJson ?? '{}'), requestId: String(item.requestId ?? ''), verifiedBy: String(item.verifiedBy ?? ''), occurredAt: String(item.occurredAt ?? '') } }
export interface OnboardingOperation { id: string; operationType: string; desiredVersion: number; status: string; attemptCount: number; errorCode: string; nextRetryAt?: string; updatedAt?: string; completedAt?: string }
export interface ApplicationOnboarding { application: Application; binding?: IdentityBinding; verifications: ApplicationVerification[]; canPublish: boolean; onboardingStatus: string; nextAction: string; blockers: string[]; roles: ApplicationRole[]; provisioningTarget?: ApplicationProvisioningTarget; onboardingChecks: OnboardingCheck[]; latestOperation?: OnboardingOperation; oneTimeClientSecret?: string; enrollmentToken?: string; enrollmentExpiresAt?: string }
...
export async function getApplicationOnboarding(id: string | number): Promise<ApplicationOnboarding> { const data = record(await apiFetch<unknown>(`/admin/portal/applications/${encodeURIComponent(String(id))}/onboarding`)); const operation = data.latestOperation ? record(data.latestOperation) : undefined; return { application: mapApplication(record(data.application)), binding: mapIdentityBinding(data.binding), verifications: listFrom(data, 'verifications').map(mapVerification), canPublish: Boolean(data.canPublish), onboardingStatus: String(data.onboardingStatus ?? 'DRAFT'), nextAction: String(data.nextAction ?? ''), blockers: Array.isArray(data.blockers) ? data.blockers.map(String) : [], roles: listFrom(data, 'roles').map(mapApplicationRole), provisioningTarget: data.provisioningTarget ? mapApplicationProvisioningTarget(data.provisioningTarget) : undefined, onboardingChecks: listFrom(data, 'onboardingChecks').map(mapOnboardingCheck), latestOperation: operation ? { id: String(operation.id ?? ''), operationType: String(operation.operationType ?? ''), desiredVersion: Number(operation.desiredVersion ?? 0), status: String(operation.status ?? ''), attemptCount: Number(operation.attemptCount ?? 0), errorCode: String(operation.errorCode ?? ''), nextRetryAt: operation.nextRetryAt ? String(operation.nextRetryAt) : undefined, updatedAt: operation.updatedAt ? String(operation.updatedAt) : undefined, completedAt: operation.completedAt ? String(operation.completedAt) : undefined } : undefined } }
export interface IdentityBindingInput { providerKey: string; protocol: string; providerApplicationRef: string; publicClientId: string; issuer?: string; redirectUris?: string[]; environments?: Array<{ key: string; name: string; redirectUris: string[] }>; scopes?: string[]; approvalId?: string; expectedConfigVersion?: number; credentialDeliveryMode?: 'CLI' | 'BROWSER' }
export interface CredentialApprovalPreparation { approvalStatus: string; approverName: string; nextAction: string; providerApplicationRef: string; publicClientId: string; issuer: string; scopes: string[] }
export async function prepareApplicationCredentialApproval(id: string | number, redirectUris: string[]): Promise<CredentialApprovalPreparation> { const data = record(await apiFetch<unknown>(`/admin/portal/applications/${encodeURIComponent(String(id))}/credential-approval`, { method: 'POST', body: { redirectUris } })); return { approvalStatus: String(data.approvalStatus ?? 'PENDING'), approverName: String(data.approverName ?? ''), nextAction: String(data.nextAction ?? ''), providerApplicationRef: String(data.providerApplicationRef ?? ''), publicClientId: String(data.publicClientId ?? ''), issuer: String(data.issuer ?? ''), scopes: Array.isArray(data.scopes) ? data.scopes.map(String) : [] } }
export async function upsertApplicationIdentityBinding(id: string | number, input: IdentityBindingInput): Promise<ApplicationOnboarding> { const data = record(await apiFetch<unknown>(`/admin/portal/applications/${encodeURIComponent(String(id))}/identity-binding`, { method: 'PUT', body: input })); return { application: mapApplication(record(data.application)), binding: mapIdentityBinding(data.binding), verifications: [], canPublish: false, onboardingStatus: 'WAITING_FOR_DEPLOYMENT', nextAction: '', blockers: [], roles: [], onboardingChecks: [], oneTimeClientSecret: data.oneTimeClientSecret ? String(data.oneTimeClientSecret) : undefined, enrollmentToken: data.enrollmentToken ? String(data.enrollmentToken) : undefined, enrollmentExpiresAt: data.enrollmentExpiresAt ? String(data.enrollmentExpiresAt) : undefined } }
export async function verifyApplicationIdentity(id: string | number, expectedConfigVersion?: number): Promise<ApplicationOnboarding & { passed: boolean }> { const data = record(await apiFetch<unknown>(`/admin/portal/applications/${encodeURIComponent(String(id))}/verify`, { method: 'POST', body: { expectedConfigVersion } })); return { application: mapApplication(record(data.application)), binding: mapIdentityBinding(data.binding), verifications: listFrom(data, 'verifications').map(mapVerification), canPublish: Boolean(data.passed), passed: Boolean(data.passed), onboardingStatus: data.passed ? 'VERIFIED' : 'ACTION_REQUIRED', nextAction: '', blockers: [], roles: [], onboardingChecks: [] } }
export async function runApplicationOnboardingChecks(id: string | number, expectedConfigVersion: number): Promise<{ checks: OnboardingCheck[]; passed: boolean }> { const data = record(await apiFetch<unknown>(`/admin/portal/applications/${encodeURIComponent(String(id))}/onboarding-checks`, { method: 'POST', body: { expectedConfigVersion } })); return { checks: listFrom(data, 'checks').map(mapOnboardingCheck), passed: Boolean(data.passed) } }
...
export interface AuditLogQuery { page?: number; pageSize?: number; operator?: string; action?: string; resourceType?: string; resourceId?: string; result?: string; from?: string; to?: string }
export async function adminListAuditLogs(params: AuditLogQuery = {}): Promise<Page<AuditLog>> {
  const data = record(await apiFetch<unknown>(`/admin/audit-logs${buildQuery({ page: params.page ?? 1, page_size: params.pageSize ?? 20, operator: params.operator, action: params.action, resource_type: params.resourceType, resource_id: params.resourceId, result: params.result, from: params.from, to: params.to })}`))
...
export const queryKeys = { me: ['me'] as const, applications: (params?: unknown) => ['applications', params] as const, application: (id: string | number) => ['applications', id] as const, recent: ['recent'] as const, popular: ['popular'] as const, categories: ['categories'] as const, tags: ['tags'] as const, favorites: ['favorites'] as const, todos: ['todos'] as const, mailAccounts: ['mail', 'accounts'] as const, mailProviders: ['mail', 'providers'] as const, mailMessages: (params?: unknown) => ['mail', 'messages', params] as const, mailMessage: (id: string | number) => ['mail', 'messages', 'detail', id] as const, adminApplications: (params?: unknown) => ['admin', 'applications', params] as const, applicationRoles: (id: string | number) => ['admin', 'applications', id, 'roles'] as const, applicationProvisioningTarget: (id: string | number) => ['admin', 'applications', id, 'provisioning-target'] as const, adminUsers: ['admin', 'users'] as const, auditLogs: (params?: unknown) => ['admin', 'audit-logs', params] as const, dashboard: ['admin', 'dashboard'] as const, identityOverview: ['admin', 'identity', 'overview'] as const, applicationOnboarding: (id: string | number) => ['admin', 'identity', 'onboarding', id] as const }
