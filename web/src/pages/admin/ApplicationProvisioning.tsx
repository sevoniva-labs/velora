...
export default function ApplicationProvisioning({ application, canManage }: Props) {
  ...
  const showSecret = (secret?: string) => { if (secret) Modal.info({ title: '保存账号下发密钥', content: <Space direction="vertical" style={{ width: '100%' }}><Input.Password value={secret} readOnly visibilityToggle /><Typography.Text type="secondary">该密钥仅显示一次，请立即保存。</Typography.Text></Space>, okText: '已保存', maskClosable: false }) }
  const refresh = () => queryClient.invalidateQueries({ queryKey: queryKeys.applicationProvisioningTarget(application.id) })
  const save = useMutation({ mutationFn: (values: ProvisioningForm) => adminUpsertApplicationProvisioningTarget(application.id, { endpointUrl: values.endpointUrl, expectedConfigVersion: target.data?.configVersion, credentialDeliveryMode: 'BROWSER' }), onSuccess: async (result) => { showSecret(result.oneTimeProvisioningSecret); message.success('账号下发已配置'); setOpen(false); await refresh() }, onError: (error) => message.error(error instanceof Error ? error.message : '账号下发配置失败') })
  ...
  return <>
    <ProDescriptions column={2} loading={target.isLoading} dataSource={target.data ?? {}} extra={canManage ? <Space>{['DEGRADED', 'FAILED'].includes(target.data?.deliveryStatus ?? '') && <Button type="primary" loading={retry.isPending} onClick={() => retry.mutate()}>重新下发</Button>}<Button type={target.data ? 'default' : 'primary'} onClick={() => setOpen(true)}>{target.data ? '修改接收地址' : '配置账号下发'}</Button>{target.data && <Dropdown menu={{ items: [{ key: 'rotate', label: '轮换密钥', danger: true, onClick: rotateConfirm }] }}><Button icon={<MoreOutlined />} aria-label="更多操作" /></Dropdown>}</Space> : undefined} columns={[
