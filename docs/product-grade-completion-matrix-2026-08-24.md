...
| 接口失败与真实空数据严格区分，并提供原位重试 | 后台列表、应用详情子页、工作台指标统一 `QueryErrorState`/重试状态 | E2E 覆盖审批接口 503；不出现“暂无数据” |
| 配置发布使用业务结构，不要求管理员理解底层参数名 | 发布环境、配置领域、变更名称、配置文件、文件校验值 | E2E + 页面截图复核完成 |
...
| 可恢复发布 | 发布前镜像标签、配置副本、PostgreSQL pgdump；应用回滚不破坏新增表 | 完成；最新回滚点 `20260824T163706Z-before-2d0c8f6` |
