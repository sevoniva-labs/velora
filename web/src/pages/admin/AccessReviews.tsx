...
export default function AccessReviews() {
  ...
  const create = useMutation({ mutationFn: (values: CreateForm) => createAccessReview({ reviewerId: values.reviewerId, dueAt: values.dueAt.toISOString(), scopeType: values.scopeType, scopeId: values.scopeType === 'ALL' ? undefined : values.scopeId }), onSuccess: async () => { message.success('权限复核已发起'); setCreateOpen(false); await queryClient.invalidateQueries({ queryKey: ['admin', 'access-reviews'] }) }, onError: (error) => message.error(error instanceof Error ? error.message : '权限复核创建失败') })
  ...
  const columns: ProColumns<AccessReview>[] = [
    ...
    { title: '复核范围', dataIndex: 'scopeName', render: (_, row) => <Space size={4}><Tag>{SCOPE_LABELS[row.scopeType] ?? '全部用户'}</Tag><Typography.Text>{row.scopeName || '全部用户'}</Typography.Text></Space> },
  ...
  return <PageContainer title="权限复核">{reviews.isError ? <QueryErrorState refetch={reviews.refetch} /> : <ProTable<AccessReview> className="velora-admin-primary-table" rowKey="id" columns={columns} {...reviewTable} loading={reviews.isLoading} search={{ labelWidth: 'auto' }} pagination={{ pageSize: 20 }} toolBarRender={canManage ? () => [<Button key="create" type="primary" icon={<PlusOutlined />} onClick={() => setCreateOpen(true)}>发起复核</Button>] : false} />}
    ...
    <Drawer title={selected ? `权限复核 · ${selected.scopeName || '全部用户'}` : '权限复核'} open={Boolean(selected)} onClose={() => setSelected(undefined)} width={860} extra={canManage && !items.isError && (items.data ?? []).some((item) => !item.decision || item.decision === 'PENDING') ? <Popconfirm title="保留全部待处理权限？" description="该操作会逐项记录处理结果。" onConfirm={() => retainAll.mutate()}><Button loading={retainAll.isPending}>全部保留</Button></Popconfirm> : undefined}>{items.isError ? <QueryErrorState compact refetch={items.refetch} /> : <ProTable<AccessReviewItem> className="velora-admin-secondary-table" rowKey="id" columns={itemColumns} dataSource={items.data ?? []} loading={items.isLoading} search={false} pagination={{ pageSize: 20 }} options={false} />}</Drawer>
