...
export default function ApplicationManagement() {
  ...
  const tabs = [{ key: 'overview', tab: '概览' }, ...(canManagePortal ? [{ key: 'information', tab: '基本信息' }] : []), ...(canReadIdentity ? [{ key: 'login', tab: '登录设置' }] : []), ...(canManagePortal ? [{ key: 'roles', tab: '应用角色' }, { key: 'access', tab: '使用范围' }] : []), ...(canReadIdentity ? [{ key: 'provisioning', tab: '账号下发' }] : []), ...(canVerify || canPublish ? [{ key: 'release', tab: '上线检查' }] : []), ...(canReadHistory ? [{ key: 'history', tab: '操作记录' }] : [])]
