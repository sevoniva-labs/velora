...
- 本地 Compose 默认允许 `http://localhost:5173` 与 `http://127.0.0.1:5173`，生产必须配置精确的 `VELORA_ALLOWED_ORIGINS`。

## 明确未接入的能力

当前 Wave 1 后端没有邮件、待办、门户设置表和 Velora 自建 OIDC Provider。Web 不再访问这些不存在的旧 API：

- 邮件/待办读取显示空状态，写操作返回“尚未接入当前后端基座”，不会伪造成功。
- 门户设置使用浏览器本地存储，仅用于本地演示；生产共享配置需要后续配置服务。
