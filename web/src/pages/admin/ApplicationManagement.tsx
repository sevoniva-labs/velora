...
export default function ApplicationManagement() {
  ...
  return <PageContainer title={application.name} subTitle={application.code} onBack={() => navigate('/admin/applications')} tabList={tabs} tabActiveKey={tab} onTabChange={setTab}>
    {tab === 'overview' && application && <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <ProCard gutter={16} wrap><StatisticCard statistic={{ title: '配置进度', value: enumLabel(APP_LIFECYCLE_LABEL, application.lifecycleStatus, '草稿') }} /><StatisticCard statistic={{ title: '可使用人数', value: canManagePortal ? effectiveAccess.data?.length ?? 0 : '—' }} /><StatisticCard statistic={{ title: '账号下发', value: canReadIdentity ? provisioning.data?.deliveryStatus === 'HEALTHY' ? '正常' : ['DEGRADED', 'FAILED'].includes(provisioning.data?.deliveryStatus ?? '') ? '异常' : provisioning.data ? '等待下发' : '未配置' : '—' }} /></ProCard>
      <ProDescriptions dataSource={application} column={2} columns={[{ title: '应用名称', dataIndex: 'name' }, { title: '应用编码', dataIndex: 'code' }, { title: '配置进度', dataIndex: 'lifecycleStatus', render: (_, row) => <Tag color={APP_LIFECYCLE_COLOR[row.lifecycleStatus ?? '']}>{enumLabel(APP_LIFECYCLE_LABEL, row.lifecycleStatus, '草稿')}</Tag> }, { title: '使用状态', dataIndex: 'status', render: (_, row) => <Tag color={row.status === 'ENABLED' ? 'success' : 'default'}>{row.status === 'ENABLED' ? '可用' : '已停用'}</Tag> }, { title: '应用地址', dataIndex: 'homeUrl', render: (_, row) => row.homeUrl || '—' }, { title: '用途', dataIndex: 'description', render: (_, row) => row.description || '—' }]} />
    </Space>}
