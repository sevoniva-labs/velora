...
export interface AdminApplicationInput { code: string; name: string; description?: string; keywords?: string; icon?: string; categoryId?: string | number | null; homeUrl?: string; launchUrl?: string; ssoType: string; casdoorApplicationName?: string; casdoorClientId?: string; owner?: string; department?: string; status: string; sort?: number; isFeatured?: boolean; healthCheckEnabled?: boolean; healthCheckUrl?: string; tagIds?: (string | number)[]; policies?: { policyType: string; value: string }[] }
...
const SETTINGS_KEY = 'velora.portal.settings'
...
export function getPortalSettings(): Promise<{ key: string; value: string }[]> { if (typeof window === 'undefined') return Promise.resolve(defaultSettings); try { const value = JSON.parse(window.localStorage.getItem(SETTINGS_KEY) ?? 'null'); return Promise.resolve(Array.isArray(value) ? value : defaultSettings) } catch { return Promise.resolve(defaultSettings) } }
export async function updatePortalSetting(key: string, value: string): Promise<unknown> { const settings = (await getPortalSettings()).filter((item) => item.key !== key); settings.push({ key, value }); if (typeof window !== 'undefined') window.localStorage.setItem(SETTINGS_KEY, JSON.stringify(settings)); return { key, value } }
