...
export default function ApplicationProvisioning({ application }: Props) {
  ...
  return <>
    <ProDescriptions column={2} loading={target.isLoading} dataSource={target.data ?? {}} extra={<Space>{['DEGRADED', 'FAILED'].includes(target.data?.deliveryStatus ?? '') && <Button type="primary" loading={retry.isPending} onClick={() => retry.mutate()}>重新同步</Button>}<Button type={target.data ? 'default' : 'primary'} onClick={() => setOpen(true)}>{target.data ? '编辑配置' : '配置账号同步'}</Button>{target.data && <Dropdown menu={{ items: [{ key: 'rotate', label: '轮换密钥', danger: true, onClick: rotateConfirm }] }}><Button icon={<MoreOutlined />} aria-label="更多操作" /></Dropdown>}</Space>} columns={[
