...
export default function ApplicationAccess({ applicationId }: Props) {
  ...
  const grantColumns: ProColumns<ApplicationAccessGrant>[] = [
    ...
    { title: '操作', valueType: 'option', width: 120, render: (_, row) => <Space><Button type="text" icon={<EditOutlined />} aria-label="编辑" onClick={() => { setEditing(row); setSelectedSubjectName(row.subjectName); setSubjectType(row.subjectType); setEffect(row.effect); setGrantOpen(true) }} /><Popconfirm title="移除此访问规则？" onConfirm={() => { setDrafts((current) => current.filter((item) => item.id !== row.id)); setDirty(true) }}><Button type="text" danger icon={<DeleteOutlined />} aria-label="移除" /></Popconfirm></Space> },
  ]
  ...
  return <Space direction="vertical" size={16} style={{ width: '100%' }}>
    ...
    <ProTable<ApplicationEffectiveAccess> headerTitle="当前可使用人员" rowKey="userId" columns={effectiveColumns} dataSource={effective.data ?? []} loading={effective.isLoading} search={{ labelWidth: 'auto' }} pagination={{ pageSize: 20 }} />
