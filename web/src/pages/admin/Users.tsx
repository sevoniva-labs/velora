...
import { ModalForm, PageContainer, ProFormSelect, ProFormText, ProTable, type ProColumns } from '@ant-design/pro-components'
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query'
import { Link } from 'react-router-dom'
import { adminCreateUser, adminListUsers, adminUpdateUserStatus, type CreateAdminUserInput } from '../../api/api'
...
export default function Users() {
  ...
  const create = useMutation({
    ...
    onSuccess: async () => { message.success('用户已创建'); setCreateOpen(false); await queryClient.invalidateQueries({ queryKey: ['admin', 'users'] }) },
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: async (_, values) => { message.success(values.status === 'ACTIVE' ? '用户已启用' : '用户已停用'); await queryClient.invalidateQueries({ queryKey: ['admin', 'users'] }) },
    onError: (error) => message.error(error instanceof Error ? error.message : '用户状态更新失败'),
  })
  const columns: ProColumns<AdminUser>[] = [
    { title: '用户', dataIndex: 'displayName', render: (_, row) => <Space direction="vertical" size={0}><Link to={`/admin/users/${row.id}`}><Typography.Text strong>{row.displayName || row.loginName}</Typography.Text></Link><Typography.Text type="secondary">{row.loginName}</Typography.Text></Space> },
    { title: '邮箱', dataIndex: 'email', render: (_, row) => row.email || '—' },
    { title: '平台角色', dataIndex: 'roles', valueType: 'select', valueEnum: Object.fromEntries((roles.data ?? []).map((role) => [role.key, role.name])), render: (_, row) => row.roles.length ? row.roles.map((role) => <Tag key={role}>{roleNames.get(role) ?? role}</Tag>) : '—' },
  ...
  return <PageContainer title="用户">
    <ProTable<AdminUser> rowKey="id" columns={columns} dataSource={users.data ?? []} loading={users.isLoading} search={{ labelWidth: 'auto' }} pagination={{ pageSize: 20, showSizeChanger: true }} toolBarRender={() => [<Button key="create" type="primary" icon={<PlusOutlined />} onClick={() => setCreateOpen(true)}>新建用户</Button>]} />
