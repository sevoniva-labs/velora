...
export default function Organization() {
  ...
  return <PageContainer title="组织架构" tabList={tabs} tabActiveKey={tab} onTabChange={setTab} extra={tab === 'organization' && canManageOrganization ? <Button type="primary" onClick={() => setOrganizationOpen(true)}>编辑组织信息</Button> : undefined}>
    ...
    {tab === 'departments' && (departments.isError ? <QueryErrorState refetch={departments.refetch} /> : <ProTable<Department> className="velora-admin-primary-table" rowKey="id" loading={departments.isLoading} {...departmentTable} columns={departmentColumns} search={{ labelWidth: 'auto' }} pagination={false} toolBarRender={canManageDepartments ? () => [<Button key="create" type="primary" icon={<PlusOutlined />} onClick={() => { setEditingDepartment(undefined); setDepartmentOpen(true) }}>新建部门</Button>] : false} />)}
    {tab === 'positions' && (positions.isError || departments.isError ? <QueryErrorState refetch={() => Promise.all([positions.refetch(), departments.refetch()])} /> : <ProTable<Position> className="velora-admin-primary-table" rowKey="id" loading={positions.isLoading || departments.isLoading} {...positionTable} columns={positionColumns} search={{ labelWidth: 'auto' }} pagination={false} toolBarRender={canManagePositions ? () => [<Button key="create" type="primary" icon={<PlusOutlined />} onClick={() => { setEditingPosition(undefined); setPositionOpen(true) }}>新建岗位</Button>] : false} />)}
