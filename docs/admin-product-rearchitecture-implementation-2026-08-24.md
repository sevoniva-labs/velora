...
- Server 与 Migrate 基础制品版本：`ab0f83c`；Worker 最终版本：`7505112`；Server 与 Web 最终版本：`92fdc4f`。最终版本已包含分类与标签管理归位、旧访问策略写入退役、生产应用硬删除禁用、门户展示假 API 移除、用户和应用目录的服务端分页、检索和筛选、Turnstile 有界失败恢复，以及审计日志的服务端分页和多条件调查筛选。
...
- 通过一次性生产验收令牌完成认证管理 API 验收：6 个平台角色、3 个用户、2 个应用均可读取；`carson` 的有效应用权限可解释；Spectra 账号下发重试保持 `HEALTHY`；旧单用户 entitlement 和旧访问策略写接口均返回 400，生产应用物理删除返回 400，确认旧写入面已退役。最终版本进一步验证 `/api/v1/admin/users?page=1&page_size=1&keyword=carson&status=ACTIVE` 返回唯一的 `carson`，并验证 `/api/v1/admin/portal/applications?page=1&page_size=1&keyword=spectra&status=ENABLED` 返回唯一的 `Spectra`；两者均返回 `total=1`、`page=1`、`page_size=1`，证明检索、状态筛选与服务端分页在公网生产链路生效。验收令牌随后删除，数据库计数为 0，管理员强制改密标志恢复。
- Server 发布后未发现 error、panic 或 fatal 日志。Worker 在未配置 WORM 归档时明确记录 `WARN` 并禁用清理，不会误报故障，也不会在没有不可变归档时删除审计数据。
- 发布前镜像保留为 `rollback-pre-ab0f83c`、`rollback-pre-ea8478d`、`rollback-pre-7505112`、`rollback-pre-18600b1`、`rollback-pre-e79c546`、`rollback-pre-d6e1cb1`、`rollback-pre-13d56d5`、`rollback-pre-d1f35ce`、`rollback-pre-bdb9133` 和 `rollback-pre-92fdc4f`。最终 Server 与 Web 制品位于 `/opt/velora/prod/releases/92fdc4f`。最终 Server SHA-256 为 `bac3ac3066b7529e947ebee65925ce57ecb38d23c146f0ab2d5ce620d3a3b304`，Worker SHA-256 为 `0cdd6a13ba15249c64c241be0ef001cc8166072feb828cb0ce9ff1e6e60c22d1`，Web 压缩制品 SHA-256 为 `d3191dea749e0aed25c06e235f917a96bf96251e9e3b4c5953b45db0b4a1e1e9`；生产 Web 入口加载 `assets/index-BGVVwySW.js`。
...
- Server、Web、Worker、OIDC Demo、Redis、Casdoor、Edge、PostgreSQL 全部健康；生产 Server 二进制 SHA-256 与 `92fdc4f` 制品一致，Web 入口为 `assets/index-BGVVwySW.js`；最新 Worker 日志仅包含预期的 WORM 未配置警告。
