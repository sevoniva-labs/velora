...
export default function IdentityIntegrations() {
  ...
  return (
    <div>
      ...
      {overview ? <Descriptions bordered size="small" column={{ xs: 1, sm: 3 }} style={{ marginBottom: 16 }}><Descriptions.Item label="连接状态"><Tag color={overview.connectionStatus === 'CONFIGURED' ? 'success' : 'warning'}>{overview.connectionStatus === 'CONFIGURED' ? '已配置' : '待配置'}</Tag></Descriptions.Item><Descriptions.Item label="Issuer">{overview.issuer || '-'}</Descriptions.Item><Descriptions.Item label="待处理应用">{overview.pendingApplicationCount}</Descriptions.Item></Descriptions> : null}
      ...
      {selected && onboarding.data ? <Card title={`${selected.name} · 接入向导`}>
        ...
        {step === 2 ? selected.ssoType === 'URL' ? <Alert type="success" showIcon message="普通链接无需身份配置" description="可直接进入验证与发布步骤。" /> : !identityTypeSupported ? <Alert type="warning" showIcon message="该接入类型暂未开放" description="SAML、CAS 和 ForwardAuth 在完成独立验收前不能保存或发布。" /> : <Form form={form} layout="vertical" onValuesChange={(_, values: IdentityDraft) => { if (!selectedId) return; try { window.localStorage.setItem(draftStorageKey(selectedId), JSON.stringify(values)) } catch { /* 浏览器存储不可用时仍可继续提交 */ } }} onFinish={(values) => bindingMutation.mutate(values)}><Descriptions bordered column={1} size="small" style={{ marginBottom: 16 }}><Descriptions.Item label="协议">OIDC Authorization Code + PKCE</Descriptions.Item><Descriptions.Item label="Scopes">openid profile email</Descriptions.Item><Descriptions.Item label="Secret">不在 Velora 输入或保存</Descriptions.Item></Descriptions><Form.Item label="身份应用引用" name="providerApplicationRef" rules={[{ required: true, message: '请输入身份中心中的应用引用' }]}><Input placeholder="例如：app-portal-demo" /></Form.Item><Form.Item label="公开 Client ID" name="publicClientId" rules={[{ required: true, message: '请输入公开 Client ID' }]}><Input /></Form.Item><Form.Item label="Issuer" name="issuer" rules={[{ required: true, message: '请输入 Issuer' }]}><Input placeholder="https://identity.example.com" /></Form.Item><Form.Item label="Redirect URI（每行一个）" name="redirectUris" rules={[{ required: true, message: '请输入回调地址' }]}><Input.TextArea rows={4} placeholder="https://app.example.com/auth/callback" /></Form.Item><Space><Button type="primary" htmlType="submit" loading={bindingMutation.isPending}>保存身份绑定</Button>{overview?.adminEntryEnabled ? <Button icon={<ExportOutlined />} onClick={() => void openConsole()}>打开身份管理控制台</Button> : null}</Space></Form> : null}
