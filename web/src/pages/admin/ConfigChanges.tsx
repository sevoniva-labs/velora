...
import { ModalForm, PageContainer, ProFormCheckbox, ProFormDigit, ProFormSelect, ProFormText, ProTable, type ProColumns } from '@ant-design/pro-components'
...
interface CreateForm { namespace: string; group: string; dataId: string; version: number; expectedPreviousVersion: number; valueDigest: string; valueRef: string; sensitive: boolean }
...
export default function ConfigChanges() {
  ...
  const columns: ProColumns<ConfigChange>[] = [
    ...
    { title: '内容引用', dataIndex: 'valueRef', search: false, ellipsis: true },
  ...
  return <PageContainer title="配置发布">
    <ProTable<ConfigChange> rowKey="id" columns={columns} dataSource={changes.data ?? []} loading={changes.isLoading} search={{ labelWidth: 'auto' }} pagination={{ pageSize: 20 }} toolBarRender={() => [<Button key="create" type="primary" icon={<PlusOutlined />} onClick={() => setCreateOpen(true)}>新建配置版本</Button>]} />
    <ModalForm<CreateForm> title="新建配置版本" open={createOpen} onOpenChange={setCreateOpen} initialValues={{ version: 1, expectedPreviousVersion: 0, sensitive: false }} submitter={{ searchConfig: { submitText: '创建', resetText: '取消' } }} onFinish={async (values) => { await create.mutateAsync(values); return true }}>
      <ProFormText name="namespace" label="命名空间" rules={[{ required: true, message: '请输入命名空间' }]} />
      <ProFormText name="group" label="配置组" rules={[{ required: true, message: '请输入配置组' }]} />
      <ProFormText name="dataId" label="配置项" rules={[{ required: true, message: '请输入配置项' }]} />
      <ProFormDigit name="expectedPreviousVersion" label="当前版本" min={0} rules={[{ required: true }]} />
      <ProFormDigit name="version" label="新版本" min={1} rules={[{ required: true }]} />
      <ProFormText name="valueDigest" label="内容 SHA-256" rules={[{ required: true, pattern: /^[a-fA-F0-9]{64}$/, message: '请输入 64 位 SHA-256' }]} />
      <ProFormText name="valueRef" label="内容引用" rules={[{ required: true, message: '请输入受控存储中的内容引用' }]} />
