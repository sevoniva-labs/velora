...
func (r *PortalRepo) ListApplications(ctx context.Context, orgID, userID string, f ApplicationFilter, includeDisabled bool) ([]portaldomain.Application, error) {
	...
	for rows.Next() {
		...
		var favorite int
		if err := rows.Scan(&item.ID, &item.OrganizationID, &item.Code, &item.Name, &item.Description, &item.Icon, &item.CategoryID, &item.CategoryName, &item.HomeURL, &item.LaunchURL, &item.LaunchType, &item.Status, &item.SortOrder, &item.Featured, &item.CreatedBy, &item.UpdatedBy, &item.CreatedAt, &item.UpdatedAt, &favorite, &item.VisitCount); err != nil {
			return nil, err
		}
...
func (r *PortalRepo) CreateApplication(ctx context.Context, orgID, actorID string, input ApplicationInput) (portaldomain.Application, error) {
	...
	err := r.db.WithinTx(ctx, func(txCtx context.Context) error {
		if _, err := r.db.ExecContext(txCtx, r.db.Rebind(`INSERT INTO portal_applications(id,organization_id,code,name,description,icon,category_id,home_url,launch_url,launch_type,status,sort_order,featured,created_by,updated_by,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`), id, orgID, input.Code, input.Name, input.Description, input.Icon, nullIfEmpty(input.CategoryID), input.HomeURL, input.LaunchURL, input.LaunchType, input.Status, input.SortOrder, input.Featured, actorID, actorID, now, now); err != nil {
			return err
		}
...
func (r *PortalRepo) listByVisitOrFavorite(ctx context.Context, orgID, userID string, limit int, favorite bool) ([]portaldomain.Application, error) {
	...
	query := `SELECT a.id,a.organization_id,a.code,a.name,a.description,a.icon,COALESCE(a.category_id,''),COALESCE(c.name,''),a.home_url,a.launch_url,a.launch_type,a.status,a.sort_order,a.featured,a.created_by,a.updated_by,a.created_at,a.updated_at,1,COALESCE(v.visit_count,0) FROM portal_applications a LEFT JOIN portal_categories c ON c.id=a.category_id AND c.organization_id=a.organization_id LEFT JOIN portal_visits v ON v.organization_id=a.organization_id AND v.user_id=? AND v.application_id=a.id WHERE a.organization_id=? AND a.status=? AND ` + condition + ` ORDER BY ` + order + ` LIMIT ?`
	...
	for rows.Next() {
		var item portaldomain.Application
		var fav int
		if err := rows.Scan(&item.ID, &item.OrganizationID, &item.Code, &item.Name, &item.Description, &item.Icon, &item.CategoryID, &item.CategoryName, &item.HomeURL, &item.LaunchURL, &item.LaunchType, &item.Status, &item.SortOrder, &item.Featured, &item.CreatedBy, &item.UpdatedBy, &item.CreatedAt, &item.UpdatedAt, &fav, &item.VisitCount); err != nil {
			return nil, err
		}
		item.Favorite = fav == 1
		out = append(out, item)
